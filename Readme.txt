...
Устройство Smart-MC0101 Вега (магнитоконтактный датчик): (приходит на порт 2 LoRaWAN)
type_packet - тип пакета
battery - заряд батареи в процентах
temperature - температура в градусах
reason_send_packet_decode - причина отправки пакета (без декода reason_send_packet)
state_in_decode - состояние входов (без декода state_in)
timestamp - время пакета

= = =

Устройство Smart-SS0102 Вега (датчик дыма): (приходит на порт 2 LoRaWAN)
type_packet_decode - тип пакета (без декода type_packet)
timestamp - время отправки показаний/пакета
current_status_decode - текущее состояние (без декода current_status)
voltage - напряжение в мВ
amperage - ток в мА
temprature_termistor - температура на термисторе в градусах
flag_use_battery_{}_decode (one,two) - флаг использования батерей (без декода flag_use_battery_{} (one,two))
flag_presence_battery_{}_decode (one,two) - флаг присутствия батарей (без декода flag_presence_battery_{} (one,two))
